...
memoize = module.exports = function (fn) {
	...
	initAsync = function (args, cb, timeoutData) {
		var waiting = cb ? [cb] : []
		  , value = { waiting: waiting }, time;
		args.push(function (err, result) {
			var args2 = arguments;
			if (timeout) {
				setTimeout(primitive ? function () { delete cache[timeoutData]; } :
					partial.call(clear, timeoutData, args.slice(0, -1)), timeout);
			}
			if (profile) {
		...
		if (primitive) {
			...
			if (cache.hasOwnProperty(id)) {
				...
				if (!mfn.preventCache) {
					...
					cache[id] = value;
					if (timeout && !async) {
						setTimeout(function () { delete cache[id]; }, timeout);
					}
				}
				delete mfn.args;
			...
			if (!mfn.preventCache) {
				...
				if (timeout && !async) {
					setTimeout(partial.call(clear, alength, args), timeout);
				}
			}
			delete mfn.args;
			return async ? value.sync : (refCounter ? value.value : value);
